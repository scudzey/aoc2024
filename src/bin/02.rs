...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    Some(calc_safe_buffer(&report_list))
}
...
pub fn check_report(report: &[u32]) -> bool {
    ...
    if !is_valid(report[0], report[1], is_increasing) {
        //((report[0] - report[1]).abs() > 3) or ((report[0] - report[1]).abs() < 1) {
        return false;
    }
...
pub fn is_valid(num1: u32, num2: u32, is_increasing: bool) -> bool {
    ...
    if ((is_increasing) && num1 > num2) || (!is_increasing && num1 < num2) {
        return false;
    }
...
pub fn check_report_safe(report: &[u32]) -> bool {
    let mut is_increasing = false;
