...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    let _lines: Vec<&str> = input
        ...
        .map(|line| {
            if line.contains('|') {
                ...
                if page_rule.len() > 1 {
                    let rule: &mut Vec<u32> = rules.entry(page_rule[0]).or_default();
    ...
    print_jobs.into_iter().for_each(|job| {
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    print_jobs.into_iter().for_each(|job| {
        ...
        let current_job =
            order_by_rules(job, &filtered_rules).unwrap_or_else(|| original_job.clone());
